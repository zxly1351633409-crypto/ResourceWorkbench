...
1. 扫描你手动指定的一个文件夹或压缩包。
2. 按资源文件夹或压缩包内的子资源生成临时卡片建议。
3. 只读预览压缩包目录，用于提前判断资源类型。
4. 参考 `Z:\整合——资源管理` 的分类树，推荐更细目标分类。
5. 为卡片生成预览图缓存，辅助人工判断。
6. 输出一份 Markdown/JSON 报告，方便我们判断规则是否靠谱。
...
窗口里可以选择资源文件夹或压缩包，点击“开始只读分析”，然后查看临时卡片和报告。
如果目标分类不对，可以在窗口中先点“修改目标分类”或“标记需确认”；这只修改本次工作台建议，不会移动文件。

如果想直接看测试目录自动跑一遍，可以双击：
