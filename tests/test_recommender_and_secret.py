...
class SecretStorageTests(unittest.TestCase):
    ...
    def test_file_key(self):
        s = self._settings()
        settings_mod.save_deepseek_api_key(s, "test-only-file-value")
        self.assertEqual(settings_mod.deepseek_api_key(s), "test-only-file-value")
    ...
    def test_env_overrides_file(self):
        s = self._settings()
        settings_mod.save_deepseek_api_key(s, "test-only-file-value")
        os.environ[self.env_name] = "test-only-env-value"
        self.assertEqual(settings_mod.deepseek_api_key(s), "test-only-env-value")
